...
export default function NewExperiencePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // 1. Önce deneyimi oluştur
      const experienceResponse = await fetch("http://localhost:5000/admin/experiences", {
        method: "POST",
        headers: {
      ...
      if (!experienceResponse.ok) {
        throw new Error("Deneyim oluşturulamadı")
      }

      const experienceData = await experienceResponse.json()
      console.log('Oluşturulan deneyim:', experienceData);

      if (!experienceData.experienceId) {
        throw new Error("Deneyim ID'si alınamadı");
      }

      const experienceId = experienceData.experienceId

      // 2. Etiketleri ekle
      console.log('Gönderilen etiketler:', tags);
      for (const tag of tags) {
        const tagResponse = await fetch(`http://localhost:5000/admin/experiences/${experienceId}/tags`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "X-CSRF-Token": csrfToken,
        ...
        if (!tagResponse.ok) {
          ...
          console.error('Etiket ekleme hatası:', errorData);
          throw new Error(errorData.error || errorData.message || `Etiket eklenemedi: ${tag}`);
        }
      }
      ...
      setError("Deneyim oluşturulurken bir hata oluştu. Lütfen daha sonra tekrar deneyin.")
      setSaving(false)
    }
  }
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-anton text-3xl text-[#404040] uppercase">Yeni Deneyim</h1>
        <Link
